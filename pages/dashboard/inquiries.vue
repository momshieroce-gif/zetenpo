  ...
  <div class="inquiries-page">
    <div class="page-header">
      <div class="header-left">
        ...
        <div>
          <h1 class="page-title">Product Inquiries</h1>
          <p class="page-subtitle">Your chats and messages</p>
        </div>
      </div>
      <div></div>
    </div>
    ...
    <div class="card">
      ...
      <div v-else class="state empty">
        ...
        <p class="empty-desc">Start a new conversation with shops or support.</p>
      </div>
    </div>
...
useHead({ title: 'Inquiries | My Near Shops' });
...
const handleFirestoreError = (err: any, path?: string) => {
  ...
  if ((msg && msg.toString().includes('Missing or insufficient permissions')) || err?.code === 'permission-denied') {
    ...
    // still log other firestore errors for debugging
...
const fetchChats = async () => {
  ...
  try {
    ...
    const fetched = snap.docs.map((d: any) => ({ id: d.id, ...d.data() } as Chat));
    chats.value = fetched;
    currentPage.value = 1;
    // fetch product names and shop names for chats that reference productId/shopId
    try {
      const productIds = [...new Set(fetched.map(f => f.productId).filter(Boolean))] as string[];
      if (productIds.length && db) {
        const chunks: string[][] = [];
        for (let i = 0; i < productIds.length; i += 30) chunks.push(productIds.slice(i, i + 30));
        const prodSnaps = await Promise.all(chunks.map(ids => getDocs(query(collection(db, 'products'), where(documentId(), 'in', ids)))));
    ...
    try {
      const shopIds = [...new Set(fetched.map(f => f.shopId).filter(Boolean))] as string[];
      if (shopIds.length && db) {
        const chunks2: string[][] = [];
        for (let i = 0; i < shopIds.length; i += 30) chunks2.push(shopIds.slice(i, i + 30));
        const shopSnaps = await Promise.all(chunks2.map(ids => getDocs(query(collection(db, 'shops'), where(documentId(), 'in', ids)))));
    ...
    fetchError.value = e?.message || 'Failed to load inquiries.';
...
const closeChatModal = () => {
  showChatModal.value = false; selectedChat.value = null; messages.value = []; messageText.value = ''; editingMessage.value = null;
};
...
// Chat create/edit/delete
// Chat creation/editing removed per request

const deleteChat = async (c: Chat) => {
  deleteTargetChat.value = c;
